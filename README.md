...
generates the server key pair for the instance (if one doesn't already exist) and writes
the instance's environment file, with encryption enabled. (The `rhizosrv@.service` unit
itself is not generated by the script - it is installed by the package.)
```
rhizosrv-setuptool.sh [--start] [--enable] NAME [DIRECTORY] [PORT]
```
...
By default the script neither enables nor starts `rhizosrv@NAME.service`; it just prints
the `systemctl` commands needed to do so. Pass `--enable` to enable the service (so it
starts on login) and/or `--start` to (re)start it now - useful to apply a config change
to an already-running instance. Re-running the script with `--start`/`--enable` and the
same `NAME` is safe: it will not touch an existing key or environment file.
...
generates a client key pair (if one doesn't already exist) and writes the instance's
environment file to mount the given server. (The `rhizofs@.service` unit itself is not
generated by the script - it is installed by the package.)
```
rhizofs-setuptool.sh [--start] [--enable] NAME URL MOUNT_POINT
```
...
By default the script neither enables nor starts `rhizofs@NAME.service`; it just prints
the `systemctl` commands needed to do so. Pass `--enable` to enable the service (so it
